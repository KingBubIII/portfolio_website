...
def all_transactions_budgeted(budgets, rules):
    ...
    for line, transaction in enumerate(reader(open(CSV_PATH + "/demo.csv"))):
        if not line == 0:
            ...
            rule_match = False
            for rule in rules:
                if not rule["category"] == "Pick a Category":
                    rule["low_limit"] = float(rule["low_limit"])
                    rule["upper_limit"] = float(rule["upper_limit"])

                    if (
                        rule["low_limit"] >= float(transaction[2])
                        and rule["upper_limit"] <= float(transaction[2])
                        and (
                            rule["text_match"] in transaction[1]
                            or rule["text_match"] == "*"
                        )
                    ):
                        try:
    ...
    for budget in budgets:
        try:
            categories[budget["name"]] = (
                float(categories[budget["name"]]) / float(budget["amount"]) * 100
            )
        except KeyError as e:
